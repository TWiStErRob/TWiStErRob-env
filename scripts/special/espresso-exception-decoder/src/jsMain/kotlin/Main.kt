...
fun parse(exception: String): ExceptionResult {
    val ambiguousRe = Regex(".*(android\\.support\\.test\\.espresso\\.AmbiguousViewMatcherException): '(.*?)' matches multiple views in the hierarchy\\.\\nProblem views are marked with '(\\*\\*\\*\\*MATCHES\\*\\*\\*\\*)' below\\.\\n\\nView Hierarchy:\\n((.|\n)*)")
    val noMatchRe = Regex(".*(android.support.test.espresso.NoMatchingViewException): No views in hierarchy found matching: \\(?(.+?)\\)?\\n.*View Hierarchy:\\n((.|\n)*)")
    val noRootRe = Regex(".*(android.support.test.espresso.NoMatchingRootException): Matcher '((.|\n)*?)' did not match any of the following roots: \\[((.|\n)*)\\]\\n((.|\n)*)")
    val runtimeRe = Regex(".*(java.lang.RuntimeException): No data found matching: \\(?(.+?)\\)? contained values: <\\[((.|\n)*?)\\]>(.*)")
